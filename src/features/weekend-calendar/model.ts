...
function formatRange(first: CalendarMonth, last: CalendarMonth) {
  ...
  if (first.year === last.year) return `${first.year} 年 ${first.month} 月至 ${last.month} 月`;
  return `${first.year} 年 ${first.month} 月至 ${last.year} 年 ${last.month} 月`;
}

export function buildWeekendCalendar(events: readonly WeekendCalendarEvent[]): WeekendCalendarModel {
  const sortedEvents = events
    .map((event, sourceIndex) => ({ event, sourceIndex, parts: parseCalendarDate(event.date) }))
  ...
  const spansYears = sortedEvents.length > 0
    && sortedEvents[0].parts.year !== sortedEvents.at(-1)?.parts.year;
  const months: CalendarMonth[] = [...eventsByMonth.entries()].map(([key, monthEntries]) => {
    const { year, month } = monthEntries[0].parts;
    const daysInMonth = toUtcDate({ year, month: month + 1, day: 0 }).getUTCDate();
    const firstDayOffset = (toUtcDate({ year, month, day: 1 }).getUTCDay() + 6) % 7;
    ...
    const days = Array.from({ length: daysInMonth }, (_, index): CalendarDay => {
      ...
      const eventDescription = dayEvents.length > 0 ? `，${dayEvents.length} 个日程` : '';
    ...
    const agenda = [...eventsByDate.entries()].map(([date, dayEvents]): CalendarAgendaDay => {
