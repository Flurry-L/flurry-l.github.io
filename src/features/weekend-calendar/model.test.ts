...
import { weekendCalendarSchema } from './schema';
...
describe('buildWeekendCalendar', () => {
  it('sorts events and preserves source order for events on the same date', () => {
    ...
    expect(calendar.months.map((month) => month.key)).toEqual(['2026-02', '2026-03']);
    expect(calendar.months[1].agenda[0].events.map((event) => event.text)).toEqual([
      'second date',
      'same date, later source item',
    ]);
  ...
  it('formats a range within one year without repeating the year', () => {
    ...
    expect(calendar.months.map((month) => month.shortLabel)).toEqual(['2 月', '6 月']);
  });
  ...
  it('formats cross-year ranges and disambiguates their tabs', () => {
    ...
    expect(calendar.months.map((month) => month.shortLabel)).toEqual(['2025 / 12', '2026 / 1']);
  });

  it('builds leap-year days and a Monday-first offset using UTC', () => {
    const calendar = buildWeekendCalendar([{ date: '2024-02-29', text: 'leap day' }]);
...
describe('calendar input validation', () => {
  it('rejects impossible dates and blank copy', () => {
    expect(() => parseCalendarDate('2025-02-29')).toThrow('Invalid calendar date');
    expect(weekendCalendarSchema.safeParse({
      summary: ' ',
  ...
  it('normalizes surrounding whitespace in authored copy', () => {
    const result = weekendCalendarSchema.parse({
      summary: ' summary ',
