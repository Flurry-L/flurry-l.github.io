...
{% include embed/{bilibili}.html id='{BV1dq421F7vA}' %}
...
核函数 $\mathcal{K}(x, x'|\tau) $ 评估两个输入 $x$ 和 $x'$ 的相似度，其中 $\tau$ 是超参数。

返回越大说明越相似。

数据集 $\mathcal{D}=\{(x_i, y_i)\}_{i=0}^N$，假设 $\bar{y_i}=0$。

预测集 $\{x_i^{*}\}_{i=0}^{M}$。

真实函数 $f(x)$，噪声 $\epsilon_{i} \sim \mathcal{N}(0, \sigma_{\epsilon}^2)$，$y_i=f(x_i)+\epsilon_i$。

$K_{X, X}(i, j)=\mathcal{K}(x_i, x_j|\tau)$ 代表任意两个输入的内核相似性
...
$f^*|X^*, D\sim \mathcal{N}(K_{X^*, X}\hat{K}^{-1}_{X, X}y,K_{X^*,X^*}-K_{X^*,X}\hat{K}_{X,X}^{-1}K_{X,X^*})$

通过 $y$ 对 $f^*$ 进行切片，得到 $f^*$ 的条件分布 $p(f^*|y)$