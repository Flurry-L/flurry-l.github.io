...
**类/结构体内部**：静态成员变量由该类的所有实例共享，静态方法没有 `this` 指针，因此无法访问非静态成员。静态方法可以在没有类实例的情况下通过 `ClassName::Method()` 调用。静态成员变量必须在类外单独定义，否则会产生链接错误：
...
栈空间一般约 2MB 左右，采用简单的反向增长方式分配，速度极快，作用域结束后自动回收，几乎没有额外开销。堆空间需要手动管理（`new` / `delete`），底层通过 `malloc` 和 free list 来维护。两者的性能差异主要体现在分配（allocation）操作上，CPU cache miss 等因素通常不太显著。
...
C++ 默认的拷贝行为是浅拷贝（逐字节复制类的内存）。如果类持有堆上的资源（如指针），需要自定义拷贝构造函数进行深拷贝：
...
字符串字面值存储在内存的只读区域，不应通过指针修改。如果需要修改，应使用字符数组（会复制一份到栈上）：

```cpp
char* name = "cherno";    // 指向只读内存
// name[3] = 'a';         // 未定义行为！

char name[] = "cherno";   // 复制到栈上的数组
name[3] = 'a';            // 安全
```
...
`std::array<int, 5> a;` 是大小固定的数组，存储在栈上，带有边界检查。
